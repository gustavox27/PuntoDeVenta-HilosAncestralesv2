...
const Inventario: React.FC = () => {
  ...
  const handleColorSelect = (colorNombre: string) => {
    setTintoreriaData({ ...tintoreriaData, color: colorNombre });
  };
  ...
  const handleExportPDF = async () => {
    try {
      ...
      await ExportUtils.exportToPDF(
        ...
        'Inventario de Productos - HILOS ANCESTRALES'
      );
  ...
  const stockPorEstado = productos.reduce((acc: any[], producto) => {
    ...
    if (existente) {
      existente.value += producto.cantidad;
    } else {
      acc.push({ name: producto.estado, value: producto.cantidad });
    }
    return acc;
  }, []);
      ...
      >
        <form onSubmit={handleTintoreriaSubmit} className="space-y-4">
          <div>
            ...
            >
              <option value="Madejas Crudas">Madejas Crudas</option>
              <option value="Madejas Reteñidas">Madejas Reteñidas</option>
            </select>
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">Haga clic en el botón para ver colores disponibles</p>
          </div>
      ...
      >
        {selectedProduct && (
          <form onSubmit={handleHilanderiaSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Stock Final</label>
              <input
                type="number"
                min="0"
                required
