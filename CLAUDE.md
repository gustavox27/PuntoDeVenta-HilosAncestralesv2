...
**Pendiente relacionado (no resuelto por esta protección)**: el botón "Pagar" en `MovementHistory.tsx` (`handlePayDebt`) todavía aplica el saldo disponible agregado bajo un id `'sistema'` ficticio en vez de consumir anticipos reales uno por uno — anticipos usados por esa vía específica no generan el evento estructurado y por tanto no quedan protegidos por `check_anticipo_usage`. Cerrar ese call-site es un fix separado (diseñado, pendiente de implementar).
